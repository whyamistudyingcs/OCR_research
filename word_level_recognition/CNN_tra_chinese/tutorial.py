...
# modify arguments/hyperparameters for training
...
# Set up dataset and dataloader that feed data into model in each epoah
    ...
    def __init__(self, txt_path, num_class, transforms=None):
        ...
        with open(txt_path, 'r') as f:
            for line in f:
                ...
                line = line.strip('\n')
                images.append(line)
                labels.append(int(line.split('/')[-2]))  # append the name of directory, aka the ground truth into label
        self.images = images
        self.labels = labels
...
def classes_txt(root, out_path, num_class=None):
    ...
    with open(out_path, 'r+') as f:
        ...
        if end < num_class - 1:
            ...
            for dir in dirs:
                files = os.listdir(os.path.join(root, dir))  # extract word file in each directory
                for file in files:
                    f.write(os.path.join(root, dir, file) + '\n') # write file path into directory
